...
def gather_trajectories(input_data, hp):
    ...
    trajectory_data = {"states": {0: [], 1: []},
                        ...
                        "actor_cell_states": {0: [], 1: []},
                        "critic_hidden_states": {0: [], 1: []},
                        "critic_cell_states": {0: [], 1: []}}
    ...
    with torch.no_grad():
        ...
        for i in range(hp.rollout_steps):
            for agent_idx in range(N_AGENTS):
                ...
                if agent_idx == 1:
                    terminal = (terminal_list[0] + terminal_list[1])
        ...
        value = critic(state.unsqueeze(0).to(GATHER_DEVICE), terminal.to(GATHER_DEVICE))
        # Future value for terminal episodes is 0.
        trajectory_data["values"][agent_idx ^ 1].append(value.squeeze(1).cpu() * (1 - terminal))
...
def pad_and_compute_returns(trajectory_episodes, len_episodes, hp):
    ...
    for i in range(episode_count):
        ...
        single_padding = torch.zeros(hp.rollout_steps//2 - len_episodes[i])
