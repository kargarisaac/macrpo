...
env_config = dict(
        ...
        gforce_penalty_coeff=0.000006 * 1,
        lane_penalty_coeff=0.1,  # 0.3,
    ...
    with torch.no_grad():
        ...
        for i in range(hp.rollout_steps):
            ...
            for agent_idx in range(N_AGENTS):
                ...
                trajectory_data["actor_hidden_states"].append(actor_hidden_cell_alternating_list[agent_idx][0].squeeze(0).cpu())
                trajectory_data["actor_cell_states"].append(actor_hidden_cell_alternating_list[agent_idx][1].squeeze(0).cpu())
                ...
                # reset hidden state for the done envs
                # done_envs = (terminal == 1).nonzero().squeeze(1)
                # if done_envs.shape[0] > 0:
                #     hidden_cell = list(actor.hidden_cell)
                #     hidden_cell[0][:, done_envs, :] = torch.zeros(actor.hp.recurrent_layers, done_envs.shape[0], actor.hp.actor_hidden_size).to(GATHER_DEVICE)
                #     hidden_cell[1][:, done_envs, :] = torch.zeros(actor.hp.recurrent_layers, done_envs.shape[0], actor.hp.actor_hidden_size).to(GATHER_DEVICE)
                #     actor.hidden_cell = tuple(hidden_cell)
                # actor_hidden_cell_alternating_list[agent_idx] = actor.hidden_cell
                # actor_hidden_cell_alternating_list[agent_idx^1] = actor.hidden_cell
        ...
        ## we need to use the last obsv to calculate values for agent 0 and 
        ## do one step to calculate values for agent1
        ## switch rnn_state
...
def evaluate_env(env_cls, actor, render=False):
    """
    Evaluate policy
    """
        ...
        while len(reward_list) < 100:
            ...
            for n in range(n_envs):
                if done[n]:
                    ...
                    # reset hidden state for the done env
                    hidden_cell = list(actor.hidden_cell)
                    hidden_cell[0][:, n, :] = torch.zeros(actor.hp.recurrent_layers, 1, actor.hp.actor_hidden_size).to(GATHER_DEVICE)
                    hidden_cell[1][:, n, :] = torch.zeros(actor.hp.recurrent_layers, 1, actor.hp.actor_hidden_size).to(GATHER_DEVICE)
                    actor.hidden_cell = tuple(hidden_cell)
    ...
    while iteration < stop_conditions.max_iterations:
        ...
        for epoch_idx in range(hp.ppo_epochs):
            for batch in trajectory_dataset:
                # Get batch
                #TODO: we just use the first rnn_state from collected data. 
                # I think in addition to switching actor rnn_state in data collection phase
                # we need to separate trajs here too.
                # separate trajs -> idx // 2 ==0 , idx //2 ==1
                # feed them separately into actor and get action dist
                # concat them -> probability_ratio , ... are normal

                actor_optimizer.zero_grad()

                loc = torch.zeros([hp.recurrent_seq_len, hp.batch_size, 3])
                cov = torch.zeros([hp.recurrent_seq_len, hp.batch_size, 3, 3])
                action_probabilities = torch.zeros_like(batch.action_probabilities)
                for i in range(2): 
                    # select actor hidden_cell data for each agent
                    actor.hidden_cell = (batch.actor_hidden_states[i:i+1], batch.actor_cell_states[i:i+1])
                    # Update actor - select data of each agent
                    
                    action_dist_temp = actor(batch.states[i::2, :, :])
                    loc[i::2] = action_dist_temp.loc 
                    cov[i::2] = action_dist_temp.covariance_matrix
                    
                    action_probabilities[i::2] = action_dist_temp.log_prob(batch.actions[i::2, :, :].to("cpu")).to(TRAIN_DEVICE)

                # to merge data again -> we need to put them together in the same sequence not cat them.
                action_dist = distributions.multivariate_normal.MultivariateNormal(loc=loc, covariance_matrix=cov)
...
if TRAIN:
    ...
    RESUME_CHECKPOINT_PATH = "/home/isaac/codes/autonomous_driving/dd0_trained_model/10/"
    actor.load_state_dict(torch.load(
        RESUME_CHECKPOINT_PATH + "actor.pt"
    ))
    evaluate_env(env_cls, actor, True)
