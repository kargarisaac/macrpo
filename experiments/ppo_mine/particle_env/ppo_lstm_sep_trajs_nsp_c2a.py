...
algo_name = f"c2a_coslr_{args.lr}_"
    ...
    while iteration < stop_conditions.max_iterations:
        ...
        for epoch_idx in range(hp.ppo_epochs):
            for batch in trajectory_dataset:
                ...
                # Update critic. train it on separate trajectories of agents
                critic_optimizer.zero_grad()

                critic_loss = 0
                for i in range(N_AGENTS):
                    critic.hidden_cell = (
                        batch.critic_hidden_states[i:i+1], batch.critic_cell_states[i:i+1])
                    values = critic(batch.states[i::N_AGENTS])
                    critic_loss += F.mse_loss(
                        batch.discounted_returns[i::N_AGENTS], values)
                critic_loss /= N_AGENTS
                torch.nn.utils.clip_grad.clip_grad_norm_(
