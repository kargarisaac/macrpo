...
algo_name = f"c2c_sp_coslr_{args.lr}_relu_"
    ...
    with torch.no_grad():
        ...
        while True:
            ...
            if done:
                ...
                episode_reward = 0
                episode_steps = 0
                actor.get_init_state(1, EVAL_DEVICE)
                actor_hidden_cell_alternating_list = [actor.hidden_cell] * N_AGENTS #the initial values are zero
...
def evaluate_env_parallel(actor, render=False):
    """
    Evaluate policy
    """

    # eval_env = VecPyTorch(
    #     SubprocVecEnv([make_env(RANDOM_SEED+i, i) for i in range(15)], "fork"),
    #     GATHER_DEVICE
    # )

    eval_env = make_env2(10, asynchronous=False, benchmark=True)
    ...
    actor.get_init_state(eval_env.num_envs, GATHER_DEVICE)
    actor= actor.to(EVAL_DEVICE)
    ...
    with torch.no_grad():
        ...
        while True:
            ...
            if done[0]:
                ...
                ##TODO: check without reseting hidden state
                actor.get_init_state(eval_env.num_envs, GATHER_DEVICE)
                actor_hidden_cell_alternating_list = [actor.hidden_cell] * N_AGENTS #the initial values are zero
...
def train_model(actor,
    ...
    while iteration < stop_conditions.max_iterations:
        ...
        test_mean_reward, test_mean_col, test_mean_min_dist, test_mean_occupied_landmark = evaluate_env_parallel(actor)
...
actor_optimizer = optim.Adam(
    actor.parameters(), lr=hp.actor_learning_rate)
# actor_lr_scheduler = torch.optim.lr_scheduler.MultiStepLR(
#     optimizer=actor_optimizer, milestones=milestones, gamma=0.5)
actor_lr_scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(
        optimizer=actor_optimizer, T_max=stop_conditions.max_iterations, eta_min=1e-5)
