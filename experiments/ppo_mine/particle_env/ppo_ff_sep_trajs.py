...
RESUME_CHECKPOINT_PATH = '/home/isaac/codes/autonomous_driving/multi-agent/experiments/ppo_mine/particle_env/checkpoints/2020_07_06/ppo_ff_c1a_20:21:12/'

algo_name = f"c1a_coslr_{args.lr}_"
...
def gather_trajectories(input_data, hp):
    ...
    with torch.no_grad():
        ...
        for i in range(hp.rollout_steps):
            ...
            # Step environment
            action_array = torch.stack(action_list, axis=1).squeeze(1).cpu().numpy()
            ...
            for a in range(N_AGENTS):
                trajectory_data["terminals"][a].append(terminal[:, 0]) #terminals are similar
                trajectory_data["rewards"][a].append(torch.tensor(reward[:, a]))
    ...
    # trajectory_tensors = {key: torch.cat([torch.stack(value[a]) for a in range(N_AGENTS)], dim=1) for key, value in trajectory_data.items()}
